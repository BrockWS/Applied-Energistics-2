...
//		ApiDefinitions definitions = (ApiDefinitions) api.definitions();
...
//	}
//
//	public void initialize(@Nonnull final FMLCommonSetupEvent event, @Nonnull final File recipeDirectory )
//	{
...
//	}
	...
	{
		AELog.info("modelRegistryEvent");
		final ApiDefinitions definitions = (ApiDefinitions) Api.INSTANCE.definitions();
		definitions.getRegistry().getBootstrapComponents( IModelRegistrationComponent.class ).forEachRemaining(IModelRegistrationComponent::modelRegistration);
	}
	...
	{
		AELog.info("modelBakeEvent");
		final ApiDefinitions definitions = (ApiDefinitions) Api.INSTANCE.definitions();
		definitions.getRegistry().getBootstrapComponents( IModelBakeComponent.class ).forEachRemaining(c -> {
			c.modelBake(event.getModelManager(), event.getModelLoader(), event.getModelRegistry());
		});
	}
	...
	public void textureStitchEvent( TextureStitchEvent.Pre event )
	{
		...
		AELog.info("textureStitchEvent");
		event.addSprite(new ResourceLocation("appliedenergistics2:blocks/glass/quartz_glass_a"));
		event.addSprite(new ResourceLocation("appliedenergistics2:blocks/glass/quartz_glass_b"));
		event.addSprite(new ResourceLocation("appliedenergistics2:blocks/glass/quartz_glass_c"));
		event.addSprite(new ResourceLocation("appliedenergistics2:blocks/glass/quartz_glass_d"));
		for (ResourceLocation resourceLocation : GlassBakedModel.TEXTURES_FRAME) {
	...
	{
		AELog.info("Registering Blocks...");
		ApiDefinitions definitions = (ApiDefinitions) AEApi.instance().definitions();
		definitions.getRegistry().getBootstrapComponents( IBlockRegistrationComponent.class ).forEachRemaining( b -> b.blockRegistration( event.getRegistry() ) );
	}

	@SubscribeEvent
	public void registerItems( RegistryEvent.Register<Item> event )
	{
		AELog.info("Registering Items...");
		ApiDefinitions definitions = (ApiDefinitions) AEApi.instance().definitions();
		definitions.getRegistry().getBootstrapComponents(IItemRegistrationComponent.class).forEachRemaining(b -> b.itemRegistration(event.getRegistry()));
		// register oredicts
